...
[secretstore_signRawHash](JSONRPC-secretstore-module#secretstore_signrawhash) local client method can be used:
...
Bob can now use the triple together with the local method [`secretstore_shadowDecrypt`](JSONRPC-secretstore-module#secretstore_shadowdecrypt) to decrypt the document.
...
Here is a flow diagram that represents what calls Bob has performed either to his own OpenEthereum node or to the Secret Store.
