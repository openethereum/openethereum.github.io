...
0. `Object` - The transaction call object.
    ...
    - `data`: `Data` - (optional) 4 byte hash of the method signature followed by encoded parameters, 32 bytes each. For details see [Ethereum Contract ABI](https://github.com/ethereum/wiki/wiki/Ethereum-Contract-ABI).
...
Request
```bash
curl --data '{"method":"eth_getTransactionCount","params":["0x407d73d8a49eeb85d32cf465507dd71d507100c1","latest"],"id":1,"jsonrpc":"2.0"}' -H "Content-Type: application/json" -X POST localhost:8545
```
